...
#[test]
fn nested_fragmented_inline_owners_keep_unique_display_identity() {
    let output = render_html(
        "<div><span id=\"outer\" style=\"display:inline;background:#112233\"><em id=\"inner\" style=\"display:inline;background:#334455\">ab cd ef gh</em></span></div>",
        options(),
    )
    .unwrap();
    ...
    assert!(fragments_for_dom(&output.layout.fragments, inner).len() >= 2);
    assert!(output.display_list.has_unique_ids());
    assert!(output.display_list.validate().is_ok());
}

#[test]
fn nested_fragmented_inline_style_change_matches_fresh_render() {
    let source = "<div><span id=\"outer\" style=\"display:inline;background:#112233\"><em id=\"inner\" style=\"display:inline;background:#334455\">ab cd ef gh</em></span></div>";
    let expected = "<div><span id=\"outer\" style=\"display:inline;background:#112233\"><em id=\"inner\" style=\"display:inline;background:#778899\">ab cd ef gh</em></span></div>";
