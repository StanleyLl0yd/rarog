...
impl RenderSession {
    ...
    pub fn complete_image_decode(
        ...
        let visual_change_pending = match reference {
            Some(reference) if display_list_needs_image_refresh(&self.display_list, reference) => {
...
fn flex_container_layout_changed(before: ComputedStyle, after: ComputedStyle) -> bool {
    before.justify_content != after.justify_content
        ...
        || before.flex_direction != after.flex_direction
        || before.row_gap != after.row_gap
        || before.column_gap != after.column_gap
}

fn flex_item_layout_changed(before: ComputedStyle, after: ComputedStyle) -> bool {
...
mod tests {
    ...
    #[test]
    fn row_reverse_change_relayouts_items_from_the_main_end() {
        let source = r#"<div id="flex" style="display:flex;width:100px"><div id="first" style="width:20px;height:10px;background:#112233"></div><div id="second" style="width:30px;height:10px;background:#445566"></div></div>"#;
        let expected_source = r#"<div id="flex" style="display:flex;flex-direction:row-reverse;width:100px"><div id="first" style="width:20px;height:10px;background:#112233"></div><div id="second" style="width:30px;height:10px;background:#445566"></div></div>"#;
        let mut session = session(source, deterministic_options());
        let flex = element_with_id(session.document(), "flex");
        let first = element_with_id(session.document(), "first");
        let second = element_with_id(session.document(), "second");
        ...
        session
            ...
            .unwrap();
        let report = session.update().expect("row-reverse update succeeds");
        let expected = render_ok(expected_source, deterministic_options());
        let flex_x = fragment_for_dom(&session.layout().fragments, flex)
            .expect("flex container remains")
            .boxes
            .content_box
            .origin
            .x;
    ...
    fn wrapped_auto_height_transition_remeasures_and_stretches_the_item_line() {
        ...
        let report = session
            .update()
            .expect("wrapped auto-height update succeeds");
        let expected = render_ok(expected_source, deterministic_options());
        let flex_y = fragment_for_dom(&session.layout().fragments, flex)
            .expect("wrapped flex container remains")
            .boxes
        ...
        assert_eq!(
            fragment_for_dom(&session.layout().fragments, first)
                .expect("remeasured wrapped item remains")
                .boxes
                .border_box
                .size
                .height,
    ...
    #[test]
    fn flex_wrap_change_relayouts_the_container_into_multiple_lines() {
        let source = r#"<div id="flex" style="display:flex;width:100px"><div id="a" style="width:60px;height:10px;background:#112233"></div><div id="b" style="width:60px;height:20px;background:#445566"></div></div>"#;
        let expected_source = r#"<div id="flex" style="display:flex;flex-wrap:wrap;width:100px"><div id="a" style="width:60px;height:10px;background:#112233"></div><div id="b" style="width:60px;height:20px;background:#445566"></div></div>"#;
        let mut session = session(source, deterministic_options());
        let flex = element_with_id(session.document(), "flex");
        let second = element_with_id(session.document(), "b");
        ...
        session
            .document_mut()
            .set_attribute(flex, "style", "display:flex;flex-wrap:wrap;width:100px")
            .unwrap();
    ...
    #[test]
    fn row_gap_change_relayouts_a_wrapped_flex_container() {
        let source = r#"<div id="flex" style="display:flex;flex-wrap:wrap;width:100px"><div id="a" style="width:60px;height:10px;background:#112233"></div><div id="b" style="width:60px;height:20px;background:#445566"></div></div>"#;
        let expected_source = r#"<div id="flex" style="display:flex;flex-wrap:wrap;width:100px;row-gap:7px"><div id="a" style="width:60px;height:10px;background:#112233"></div><div id="b" style="width:60px;height:20px;background:#445566"></div></div>"#;
        let mut session = session(source, deterministic_options());
        let flex = element_with_id(session.document(), "flex");
        let second = element_with_id(session.document(), "b");
        ...
        session
            ...
            .set_attribute(
                flex,
                "style",
                "display:flex;flex-wrap:wrap;width:100px;row-gap:7px",
            )
            .unwrap();
    ...
    fn align_self_change_relayouts_the_parent_flex_row() {
        ...
        session
            ...
            .set_attribute(
                first,
                "style",
                "width:20px;height:10px;align-self:flex-end;background:#112233",
            )
            .unwrap();
        let report = session.update().expect("align-self update succeeds");
        let expected = render_ok(expected_source, deterministic_options());
