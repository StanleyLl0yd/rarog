...
use grid::{
    ...
    GridIntrinsicTrackResolveOptions, GridTrackGroupAlignment,
    resolve_grid_track_group_distribution, resolve_intrinsic_tracks_with_space,
};
    ...
    fn layout_grid_children(
        ...
        let Ok(columns) = resolve_intrinsic_tracks_with_space(
            ...
            GridIntrinsicTrackResolveOptions {
                ...
                available_space: Some(containing_block.available.width),
            ...
            .any(|track| matches!(track, GridTrackSizing::Auto | GridTrackSizing::Fraction(_)))
        {
            ...
            let Ok(rows) = resolve_intrinsic_tracks_with_space(
                ...
                GridIntrinsicTrackResolveOptions {
                    ...
                    available_space: definite_content_height,
        ...
        for (child, placement) in nodes.into_iter().zip(&layout.items) {
            ...
            let inline_alignment =
                grid_inline_self_alignment(style.justify_self, container.style.justify_items);
            let block_alignment =
            ...
            let margin_box_x = aligned_grid_item_origin(
                placement.area.origin.x,
                placement.area.size.width,
                margin_box_width,
                inline_alignment,
            );
            let margin_box_y = aligned_grid_item_origin(
                placement.area.origin.y,
                placement.area.size.height,
                margin_box_height,
                block_alignment,
            );
            ...
            let mut child_y = margin_box_y + style.margin.top;
...
fn grid_inline_self_alignment(
    justify_self: JustifySelf,
    container_justify_items: JustifyItems,
) -> GridSelfAlignment {
    match justify_self {
        JustifySelf::Auto => match container_justify_items {
            JustifyItems::Normal | JustifyItems::Stretch => GridSelfAlignment::Stretch,
            JustifyItems::Start => GridSelfAlignment::Start,
            JustifyItems::End => GridSelfAlignment::End,
            JustifyItems::Center => GridSelfAlignment::Center,
        },
        JustifySelf::Stretch => GridSelfAlignment::Stretch,
        JustifySelf::Start => GridSelfAlignment::Start,
        JustifySelf::End => GridSelfAlignment::End,
        JustifySelf::Center => GridSelfAlignment::Center,
    }
}
...
fn aligned_grid_item_origin(
    area_origin: f32,
    area_size: f32,
    margin_box_size: f32,
    alignment: GridSelfAlignment,
) -> f32 {
    match alignment {
        GridSelfAlignment::Stretch | GridSelfAlignment::Start => area_origin,
        GridSelfAlignment::End => area_origin + area_size - margin_box_size,
        GridSelfAlignment::Center => area_origin + (area_size - margin_box_size) / 2.0,
    }
}
...
mod tests {
    ...
    fn css_grid_auto_columns_stretch_across_definite_inline_space_by_default() {
        ...
        assert_eq!(grid.children[0].boxes.border_box.size.width, 300.0);
        assert_eq!(grid.children[1].boxes.border_box.origin.x, 300.0);
        assert_eq!(grid.boxes.content_box.size.width, 320.0);
    }

    #[test]
    fn css_grid_flex_start_keeps_auto_column_at_max_content_when_space_remains() {
        let mut doc = Document::new();
        let container = doc
            .append_new(
                doc.root(),
                element(
                    "div",
                    Some(
                        "display:grid;justify-content:flex-start;grid-template-columns:auto 20px;grid-template-rows:20px",
                    ),
                ),
            )
            .unwrap();
    ...
    #[test]
    fn css_grid_definite_inline_space_maximizes_auto_track_between_intrinsic_bounds() {
        let mut doc = Document::new();
        let container = doc
            .append_new(
                doc.root(),
                element(
                    "div",
                    Some(
                        "display:grid;width:60px;justify-content:flex-start;grid-template-columns:auto;grid-template-rows:20px",
                    ),
                ),
            )
            .unwrap();
        let item = doc.append_new(container, element("div", None)).unwrap();
        doc.append_new(item, NodeKind::Text("hello world".into()))
            .unwrap();
        ...
        assert_eq!(item.boxes.border_box.size.width, 60.0);
    }
    ...
    #[test]
    fn css_grid_definite_height_stretches_auto_rows_by_default() {
        let mut doc = Document::new();
        let container = doc
            .append_new(
                doc.root(),
                element(
                    "div",
                    Some(
                        "display:grid;width:40px;height:100px;grid-template-columns:40px;grid-template-rows:auto auto",
                    ),
                ),
            )
            .unwrap();
        ...
        assert_eq!(grid.children[0].boxes.border_box.size.height, 47.0);
        assert_eq!(grid.children[1].boxes.border_box.origin.y, 47.0);
        assert_eq!(grid.children[1].boxes.border_box.size.height, 53.0);
        assert_eq!(grid.boxes.content_box.size.height, 100.0);
    }

    #[test]
    fn css_grid_flex_start_keeps_definite_height_auto_rows_at_intrinsic_sizes() {
        let mut doc = Document::new();
        let container = doc
            .append_new(
                doc.root(),
                element(
                    "div",
                    Some(
                        "display:grid;width:40px;height:100px;align-content:flex-start;grid-template-columns:40px;grid-template-rows:auto auto",
                    ),
                ),
            )
            .unwrap();
    ...
    fn css_grid_justify_content_positions_and_distributes_track_group() {
        for (value, first_x, second_x) in [
            ...
            assert_eq!(
                grid.children[0].boxes.border_box.origin.x, first_x,
                "justify-content:{value}"
            );
            assert_eq!(
                grid.children[1].boxes.border_box.origin.x, second_x,
                "justify-content:{value}"
            );
        }
    }
    ...
    fn css_grid_align_content_positions_and_distributes_track_group() {
        for (value, first_y, second_y) in [
            ...
            assert_eq!(
                grid.children[0].boxes.border_box.origin.y, first_y,
                "align-content:{value}"
            );
            assert_eq!(
                grid.children[1].boxes.border_box.origin.y, second_y,
                "align-content:{value}"
            );
        }
    }
    ...
    fn css_grid_justify_items_sets_default_inline_self_alignment() {
        ...
        let container = doc
            .append_new(
                ...
                element(
                    "div",
                    Some(
                        "display:grid;grid-template-columns:60px;grid-template-rows:40px;justify-items:center",
                    ),
                ),
            )
            .unwrap();
        ...
        assert_eq!(item.boxes.border_box, Rect::new(20.0, 0.0, 20.0, 10.0));
    }

    #[test]
    fn css_grid_justify_self_overrides_container_justify_items() {
        let mut doc = Document::new();
        let container = doc
            .append_new(
                doc.root(),
        ...
        doc.append_new(
            container,
            element("div", Some("width:20px;height:10px;justify-self:end")),
        )
        .unwrap();
        ...
        assert_eq!(item.boxes.border_box, Rect::new(40.0, 0.0, 20.0, 10.0));
    }

    #[test]
    fn css_grid_justify_items_non_stretch_auto_width_uses_intrinsic_size() {
        let mut doc = Document::new();
        let container = doc
            .append_new(
                doc.root(),
            ...
            .unwrap();
        let item = doc
            .append_new(container, element("div", Some("height:10px")))
            .unwrap();
        doc.append_new(item, NodeKind::Text("hello".into()))
            .unwrap();
    ...
    fn css_grid_item_self_alignment_positions_explicit_size_inside_area() {
        ...
        assert_eq!(item.boxes.border_box, Rect::new(20.0, 15.0, 20.0, 10.0));
    }

    #[test]
    fn css_grid_item_end_alignment_preserves_physical_margins() {
        let mut doc = Document::new();
        let container = doc
            .append_new(
                doc.root(),
        ...
        doc.append_new(
            ...
            element(
                "div",
                Some(
                    "width:20px;height:10px;margin:2px 3px 4px 5px;justify-self:end;align-self:end",
                ),
            ),
        )
        .unwrap();
    ...
    #[test]
    fn css_grid_auto_size_still_stretches_by_default() {
        let mut doc = Document::new();
        let container = doc
            .append_new(
                doc.root(),
        ...
        assert_eq!(item.boxes.border_box, Rect::new(0.0, 0.0, 60.0, 40.0));
    }
