...
pub fn parse(input: &str) -> Document {
    ...
    while i < bytes.len() {
        if bytes[i] == b'<' {
            if let Some(close_rel) = input[i..].find('>') {
                ...
                if raw.starts_with('/') {
                    ...
                    if !tag.is_empty() {
                        ...
                        let id = doc.append(parent, NodeKind::Element(ElementData {
                            tag_name: tag,
                            attributes: attrs,
                        }));
            ...
            let text = input[i..next].split_whitespace().collect::<Vec<_>>().join(" ");
            if !text.is_empty() {
                let parent = *stack.last().expect("document stack is never empty");
...
fn parse_tag(input: &str) -> (String, BTreeMap<String, String>) {
    ...
    while cursor < rest.len() {
        while cursor < rest.len() && rest.as_bytes()[cursor].is_ascii_whitespace() { cursor += 1; }
        if cursor >= rest.len() { break; }
        let name_start = cursor;
        while cursor < rest.len() {
            let b = rest.as_bytes()[cursor];
            if b == b'=' || b.is_ascii_whitespace() { break; }
            cursor += 1;
        }
        ...
        if cursor < rest.len() && rest.as_bytes()[cursor] == b'=' {
            cursor += 1;
            while cursor < rest.len() && rest.as_bytes()[cursor].is_ascii_whitespace() { cursor += 1; }
            if cursor < rest.len() && (rest.as_bytes()[cursor] == b'"' || rest.as_bytes()[cursor] == b'\'') {
                let quote = rest.as_bytes()[cursor];
                cursor += 1;
                let start = cursor;
                while cursor < rest.len() && rest.as_bytes()[cursor] != quote { cursor += 1; }
                value = rest[start..cursor].to_string();
                if cursor < rest.len() { cursor += 1; }
            } else {
                let start = cursor;
                while cursor < rest.len() && !rest.as_bytes()[cursor].is_ascii_whitespace() { cursor += 1; }
                value = rest[start..cursor].to_string();
...
fn matches_void(node: &rarog_dom::Node) -> bool {
    match &node.kind {
        NodeKind::Element(el) => matches!(el.tag_name.as_str(), "br" | "hr" | "img" | "input" | "meta" | "link"),
        _ => false,
    }
}
