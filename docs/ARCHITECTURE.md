...
On Windows, `WindowsSiteProcess` binds a real child lifetime to a live Host-produced `SiteLease`. Non-blocking observation, blocking wait and explicit termination all feed the child's exit into `HostControlPlane::process_lost` once. The adapter redirects standard streams to null and uses the Windows no-console creation flag, while `std::process::Child` and all OS process details stay private to `rarog-platform-windows`. Dropping a still-live adapter is emergency containment: it kills and reaps the child, but coherent recovery uses the explicit Host-reporting paths. This is a lifecycle boundary, not the R4 sandbox policy. See ADR-0108.
