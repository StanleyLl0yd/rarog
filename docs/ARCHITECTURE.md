...
This Workers / Service Workers foundation still does not implement Service Worker Fetch interception, Host/navigation-context request routing, script-visible Service Worker APIs/events, Cache Storage, push/sync, SharedWorker semantics, generic Web structured-clone completeness, transferables or OS thread/process isolation. Those remain separate R5 slices.
