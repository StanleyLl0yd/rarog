...
- Subclass installation is accepted only on the HWND owner thread, as required by the Windows subclass helper contract, and an existing Rarog subclass on the same HWND is rejected instead of silently replacing its `dwRefData`. A private per-HWND Win32 property records that attachment; the bridge does not introduce a process-global HWND registry. Wrapper destruction first invalidates retained provider state. Same-thread destruction removes and releases the subclass context and property immediately; cross-thread destruction performs no forbidden subclass-helper call and leaves only the inert callback-owned context for terminal `WM_NCDESTROY` cleanup.
