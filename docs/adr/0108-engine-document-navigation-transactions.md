...
A matching `FetchResponse` is consumed only by the current transaction. A backend-supplied final URL different from the engine request URL is rejected because it would indicate redirect/final-URL policy was performed below Rarog. Redirect responses are explicit failures until Rarog-owned redirect processing issues the next transport request.
